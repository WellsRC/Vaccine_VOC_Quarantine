...
for TestN=1:NumTests
    ...
    for VAC=0:1
        parfor jj=1:60 
            IDSLS(jj)=((1./IncPv(jj)).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 qv(jj)],testtype,R0S(jj),R0A(jj),0,IncPv(jj),(IncPv(jj)+20),SelfIsolate,VAC,VOCv(jj)),0,IncPv(jj),qv(jj),inf));
            IDSLA(jj)=((1./(IncPv(jj)+20)).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 qv(jj)],testtype,R0S(jj),R0A(jj),1,IncPv(jj),(IncPv(jj)+20),0,VAC,VOCv(jj)),0,(IncPv(jj)+20),qv(jj),inf));  
        end

        save(['TestingonEntryExit_' testName{TestN} '_Vaccinated=' num2str(VAC) '.mat']);
    end
end